...
### Un error que esta verificación no detectó

Al correr el playbook completo contra la VM real de Azure por primera vez, el paso de instalar los paquetes de Docker falló con `No package matching 'docker-ce' is available`. La causa fue un error de precedencia de operadores en la línea que armaba el repositorio apt: `ansible_architecture == 'x86_64' | ternary('amd64', 'arm64')`. En Jinja2 el filtro `| ternary` se aplica primero al texto `'x86_64'`, no al resultado de la comparación completa, así que la expresión terminaba evaluando a `false` en vez de a `'amd64'`, y el archivo del repositorio quedaba con `arch=False`, una arquitectura que ningún paquete puede satisfacer. La verificación descrita arriba no detectó esto porque escribió `arch=amd64` directamente en el comando de prueba manual, sin pasar por esa línea del rol.

Se corrigió reemplazando esa expresión por `dpkg --print-architecture` (el mismo comando que usa la documentación oficial de Docker), registrado con `ansible.builtin.command` y usado directamente en la plantilla del repositorio, sin ninguna traducción manual del nombre de la arquitectura. Se volvió a verificar el rol completo, esta vez incluyendo el archivo real del proyecto (`include_role`, no una copia escrita a mano) contra un contenedor limpio de `ubuntu:24.04`: el paso de instalar los paquetes, que antes fallaba, ahora termina en `changed`, sin error.

El playbook completo, con ambos roles encadenados, ya se corrió contra la VM real de Azure en ese mismo ciclo, junto con las capturas finales de `docs/TERRAFORM.md` (ver sección 4 de ese documento): el frontend quedó accesible desde la IP pública de la VM, con login y los todos cargando correctamente.
